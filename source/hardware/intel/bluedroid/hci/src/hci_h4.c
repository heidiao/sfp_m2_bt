...
#define HCI_INTEL_STARTUP_EVT           0x00
#define HCI_INTEL_RESET_CMD             0xFC01
#define HCI_INTEL_STARTUP_EVT           0x00
#define HCI_INTEL_RESET_CMD             0xFC01
...
uint8_t internal_event_intercept(void)
{
    ...
    else if (event_code == HCI_INTEL_VSC_EVT)
    {
        ...
        if (p_int_evt_cb)
        {
            ...
            /*
             * For B0, Intel Reset command complete event is not receiving. Hence using
             * Intel startup event to dequeue the Intel Reset command from command buffer
             */
            else if((sub_event_code == HCI_INTEL_STARTUP_EVT) &&
                    (p_cb->int_cmd[p_cb->int_cmd_rd_idx].opcode == HCI_INTEL_RESET_CMD))
            {
                p_cb->int_cmd_rd_idx = ((p_cb->int_cmd_rd_idx+1) & \
                                        INT_CMD_PKT_IDX_MASK);
                p_cb->int_cmd_rsp_pending--;
